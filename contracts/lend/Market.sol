...
contract Market is IMarket {
    ...
    constructor(IERC20 _token, uint _baseBorrowAnnualRate, uint _blocksPerYear, uint _utilizationRateFraction) {
    ...
    function supplyOf(address user) public override view returns (uint) {
        return supplies[user].supply;
    }

    function borrowBy(address user) public override view returns (uint) {
        return borrows[user].principal;
    }

    function updatedBorrowBy(address user) public override view returns (uint) {
    ...
    function updatedSupplyOf(address user) public override view returns (uint) {
    ...
    function transferTo(address sender, address receiver, uint amount) public override onlyController {
        require(amount > 0);
